...
module.exports = () => {
  Route.post("authorise", "AuthController.authorise").validator(
    "auth/Authorise"
  );
  ...
  Route.post("forgot/password", "AuthController.forgotPassword").validator(
    "auth/ForgotPassword"
  );
  ...
  Route.post("refresh/token", "AuthController.refreshToken").validator(
    "auth/RefreshToken"
  );
  Route.get("revoke/token", "AuthController.revokeToken").middleware("jwtAuth");

  Route.get("remove/user/:id", "AuthController.removeUser").middleware(
    "jwtAuthSuperAdmin"
  );

  Route.post("update/email", "AuthController.updateEmail")
    .middleware("jwtAuth")
    .validator("auth/UpdateEmail")
    .as("auth.update.email");
  Route.post("update/password", "AuthController.updatePassword")
    .middleware("jwtAuth")
    .validator("auth/UpdatePassword")
  ...
  Route.post("assign/role", "AuthController.assignRole")
    .validator("auth/AssignRole")
    .middleware("jwtAuthSuperAdmin");
  Route.get("toggle/user/ban/:id", "AuthController.toggleUserBan").middleware(
    "jwtAuthSuperAdmin"
  );
  ...
  /*  #######  Pages with view  ######### */
  Route.get(
    "view/confirm/email/:token",
    "AuthController.confirmEmailRender"
  ).as("auth.confirm.email");
  Route.get(
    "view/reset/password/:token",
    "AuthController.resetPasswordForm"
  ).as("auth.reset.password.form");
};
